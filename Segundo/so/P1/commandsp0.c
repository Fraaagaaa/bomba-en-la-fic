...
char* modetostring(int mode)
{
    ...
    if((mode & O_ACCMODE) == O_RDONLY) strcpy(buf, "O_RDONLY");
...
void cmd_authors(char **args)
{
    if(args[1] == NULL)
    {
        printf("Authors: Miguel Fraga Pico, Lucas Marqués Núñez\n");
        printf("Login: miguel.fraga.pico, lucas.marques\n");
        return;
    }
    else if(strcmp(args[1], "-n") == 0) printf("Authors: Miguel Fraga Pico, Lucas Marqués Núñez\n");
    else if(strcmp(args[1], "-l") == 0) printf("Login: miguel.fraga.pico, lucas.marques\n");
    else
    {
