...
int main(int argc, char **argv, char **envp) 
{
    ...
    while(true)
    {
        prompt();
        read_input(command);
        handle_input(command, &history, &opened_files, &dirparams, &memory_list, &processList, envp);
    }
...
void handle_input(char *command, List *history, List *opened_files, struct DirParams *params, MemList *memory_list, List *processList, char **envp)
{
    char *args[64];
    ...
    // P3
    else if(!strcmp(args[0], "uid")) cmd_uid(args);
    else if(!strcmp(args[0], "envvar")) cmd_envvar(args, envp);
    else if(!strcmp(args[0], "showenv")) cmd_showenv(args, envp);
    else if(!strcmp(args[0], "fork")) cmd_fork(args);
    else if(!strcmp(args[0], "exec")) cmd_exec(args);
    else if(!strcmp(args[0], "jobs")) cmd_jobs(processList);
    else if(!strcmp(args[0], "deljobs")) cmd_deljobs(args, processList);
    else cmd_execute_external(args, processList);
}
